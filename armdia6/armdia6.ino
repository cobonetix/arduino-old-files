...
MOTOR motors[MOTORS_DEFINED] = { {GPA6,GPA7,7,0x40,19.0,MOTOR_IDLE,0,0}, {GPA4,GPA5,8,0x80,15.9,MOTOR_IDLE,0,0}, {GPA2,GPA3,10,0x20,15.9,MOTOR_IDLE,0,0}  }; 
...
int moveToPosition(char motor,int position)
{
     int degreeChange;
        ...
        degreeChange = position - motors[motor].position/motors[motor].ticksPerDegree;
               
        if (degreeChange  >= 0)
        {
          startMotor(motor,ROT_CW,degreeChange * motors[motor].ticksPerDegree,0);
        }
        else
        {
          startMotor(motor,ROT_CCW,abs(degreeChange * motors[motor].ticksPerDegree),0);
