...
#include <stdio.h>
//#include <D:\software\BobsPickerRobot\bobsrobotheaderfile.h>
...
MOTOR motors[MOTORS_DEFINED] = { {GPA6,GPA7,7,A6,0x40,19.0,4.0,0.0,MOTOR_IDLE,0,0}, {GPA4,GPA5,8,A0,0x80,15.9,4.0,0.0,MOTOR_IDLE,0,0}, {GPA2,GPA3,10,A1,0x20,15.9,3.9,0.0,MOTOR_IDLE,0,0}  }; 
...
int servoGpio [NUM_SERVOS] = {9,6,5,3};  // all these pins can do PWM the last two are always moved together
int servoAnalog[NUM_SERVOS] = {3,2,0,0};  // only two have analog inputs

#define SERVO_FIX  0
#define SERVO_FLAT  94
...
double* getcurrentwristposition(double angle1, double angle2, double angle3, double H) {
  ...
  double nine = 9.0;
	...
	myposition = getmyarmpoints(nine, nine, angle1, angle2, H, double(0)); // 9 and 9 are the length of the bicept and arm and needs to be gloalized.
...
void* gotopoint(double currentposition[], double newposition[], double ll1, double ll2, int leftrightbit, double tm4, double newtm4)
{
	...
	double m = int(del_tm / anginc);

	int n;
	

	for (n = 1; n <= abs(int(del_x)); ++n) {
		currentposition[0] += 1 * (signbit(del_x) ? -1 : 1);
		Robot_Plan = elbow2(currentposition, ll1,ll2, Curr_tm, leftrightbit);
	  addTrajectoryPoint(Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);    
    printf("x   %.2f, %.2f, %.2f", Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);
		printf("\n");
		
	};
	for (n = 1; n <= abs(int(del_y)); ++n) {
		currentposition[1] += 1 * (signbit(del_y) ? -1 : 1);
		Robot_Plan = elbow2(currentposition, ll1, ll2, Curr_tm, leftrightbit);
	  addTrajectoryPoint(Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);
		printf("y   %.2f, %.2f, %.2f", Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);
		printf("\n");
	...
	for (n = 1; n <= abs(m); ++n) {
		...
		Robot_Plan = elbow2(currentposition, ll1, ll2, Curr_tm, leftrightbit);
	  addTrajectoryPoint(Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);
		printf("r   %.2f, %.2f, %.2f", Robot_Plan[0], Robot_Plan[1], Robot_Plan[2]);
		printf("\n");
	};
...
double* getmycurrentwristposition(double angle1, double angle2, double angle3, double H) {
  ...
  double nine = 9.0;
	...
	myposition = getmyarmpoints(nine, nine, angle1, angle2, H, 0); // 9 and 9 are the length of the bicept and arm and needs to be gloalized.
...
double* elbow2(double Pef[], double r1, double r2, double tm4, int myleftrightbit) {
	...
	double BobsSholderAngle = 0;
	double BobsElbowAngle = 0;
	int quadbit = 0;
	double x1 = 0;
	double y1 = 0;
	double x2 = Pef[0];
	...
	if (myleftrightbit == 0) {
		...
		if (left_x >= 0 and left_y >= 0) {
			if (abs(left_x) <= 0.0001) {
				left_x = 0.00001;
			};
			BobsSholderAngle = 270 - k * atan(abs(left_y) / abs(left_x));
			...
			BobsSholderAngle = 90 + k * atan(abs(left_y) / abs(left_x));
			BobsElbowAngle = 360 - k * myloc(r1, r2, d);
			quadbit = 2;
		...
		if (right_x >= 0 and right_y >= 0) {
			...
			BobsSholderAngle = 270 - k * atan(abs(right_y) / abs(right_x));
			BobsElbowAngle = k * myloc(r1, r2, d);
			quadbit = 11;
		}
		else if (right_x < 0 and right_y > 0) {
			if (abs(right_x) <= 0.0001) {
				right_x = -0.00001;
			};
			BobsSholderAngle = 90 + k * atan(abs(right_y) / abs(right_x));
			BobsElbowAngle = k * myloc(r1, r2, d);
			quadbit = 12;
		}
		else if (right_x < 0 and right_y < 0) {
	...
	my_DATA[2] = tm4;
	//my_DATA[3] = quadbit;
	//printf("%f, %f, %f, %f, %d\n\n", my_DATA[0], my_DATA[1], my_DATA[2], my_DATA[3], quadbit);
...
void prt3d(char *pc,double v1, double v2, double v3)
{
  Serial.print(pc);
  Serial.print(v1);
  Serial.print(' ');
...
int runCommand() {
  ...
  case '6':
    ...
    case 'r':
        ...
        endpt = 180 -(arg2 & 0xff);
        servo[servoR].write(endpt);
        break;
    
    case 'l':

        endpt = (arg2 & 0xff);
        servo[servoL].write(endpt);
        break;
  ...
  case '7':
      ...
      Serial.println( "OK - L: " + String(analogRead(servoAnalog[servoL])) + " R: " + String(analogRead(servoAnalog[servoR])));
  ...
  case ROT_HALT:
      for (i = 0; i < MOTORS_DEFINED; i++)
      {
        ...
        mcp.digitalWrite(motors[i].enGpio ,HIGH);
        Serial.println("OK"); 
      }
      break; 
     ...
     if (argv1[0] == 'a')
    {
      ...
      if (argv1[0] == 'p')
      {
         ...
         // convert to x,y,rot
	       ...
	       hereismyposition = getcurrentwristposition(double(currentPosition[0]), double(mechToKinDegrees(1,currentPosition[1])), double(mechToKinDegrees(2,currentPosition[2])), double(48));  //these angles come from the spreadsheet in MatLab
  ...
  case CALIBRATE:
   ...
   case MINIARM_CALIB:
     if (argv1[0] == 'p')
     {
       ...
       for (i = 0; i < arg1; i++)
       { 
         Serial.println( String(i) + ":" + String(analogRead(servoAnalog[servoL])) + " " + String(analogRead(servoAnalog[servoR])));
         delay(500);
       }
       Serial.println( "OK");
       return;
     ...
     if (argv1[0] == 'm')
     {
        Serial.println( "set to min");
        servo[servoL].detach();
        servo[servoR].detach();
        delay(5000);// record the maximum extent
        leftMax = analogRead(servoAnalog[servoL]);
        rightMax = analogRead(servoAnalog[servoR]);
        Serial.println( "OK :" + String(leftMax) + " " + String(rightMax));
        return;
     }  
 
 #define SAMPLING_DEADZONE 10

     if (argv1[0] == 'c')
     {
        // do calibrate, first move to min position
      
          int initPosL = INIT_POS_L;
          int initPosR = INIT_POS_R;
          servo[servoL].write(initPosL);
					servo[servoR].write(180-initPosR);
					delay(500);

					Serial.println("rightMax " + String(rightMax));
          
          while(1)
          {
            int t =  analogRead(servoAnalog[servoR]);
            if (t > (rightMax - SAMPLING_DEADZONE))
            {
               Serial.println("rex inc: " + String(t) );
               break;
            }
            Serial.println("r inc: " + String(t) + ":" + String(initPosR+1) );
						servo[servoR].write(180- ++initPosR);
    				delay(100);
					}
          ...
          while(1)
          {
            ...
            if (t <= (rightMax + SAMPLING_DEADZONE))
            {
               Serial.println("rex dec: " + String(t) );
              break;
            }
            Serial.println("r dec: " + String(t) + ":" + String(initPosR-1));
						servo[servoR].write(180- --initPosR);
    				delay(100);
					}
          ...
          //return;
					...
					Serial.println("leftMax " + String(leftMax));
          
          while(1)
          {
            int t =  analogRead(servoAnalog[servoL]);
            if (t > (leftMax + SAMPLING_DEADZONE))
            {
               Serial.println("lex dec: " + String(t) );
              break;
            }
            Serial.println("l dec: " + String(t) + ":" + String(initPosL-1));
						servo[servoL].write(--initPosL);
    				delay(50);
					}
          ...
          while(1)
          {
            ...
            if (t <= (leftMax - SAMPLING_DEADZONE))
            {
               Serial.println("lex inc: " + String(t) );
               break;
            }
            Serial.println("l inc: " + String(t) + ":" + String(initPosL+1));
						servo[servoL].write(++initPosL);
    				delay(50);
					}
          
         
          Serial.println( "OK :" + String(initPosL - INIT_POS_L) + " " + String(initPosR- INIT_POS_R));
          return;
     }
     break;
...
#define FIXED_OFFSET_R          108
   ...
   case MINIARM_MOVE:
    ...
    // _stretch = constrain(_stretch, ARM_STRETCH_MIN,   ARM_STRETCH_MAX);		// +55, set zero -stretch 
     ...
     Serial.println ("input: S:" + String(_stretch) + " H: " + String( _height));
     ...
     prt3(" servo position: ",angleL,angleR, 0);
     prt3(" servo position adj: ",angleL +93,angleR-77, 0);
    ...
    if (argv3[0] == 'g')
    {
      servo[servoL].write(angleL -93);
      servo[servoR].write(180-(angleR+77));
     } 
     Serial.println( "OK");
 break;
