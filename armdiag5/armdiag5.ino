...
MOTOR motors[MOTORS_DEFINED] = { {GPA6,GPA7,7,A0,0x40,19.0,4.0,0.0,180,525,MOTOR_IDLE,0,0}, {GPA4,GPA5,8,A1,0x80,15.9,4.0,0.0,180,418,MOTOR_IDLE,0,0}, {GPA2,GPA3,10,A2,0x20,15.9,3.9,0.0,180,550,MOTOR_IDLE,0,0}  }; 
  ...
  for (m=0; m < MOTORS_DEFINED;m++)
  {
    if (motors[m].direction != MOTOR_IDLE)
    {
     ...
     // Serial.println(mcpPortin,HEX);
      ...
      if ( ((mcpPortin & motors[m].zeroMask) == 0) && (motors[m].ticksPerformed > motors[m].beginZeroMonitoring)   )
      {
      ...
      if(motors[m].direction == MOTOR_CW) 
      {
        ...
        if (m == 2)
        {
          if (motors[m].currentAv >= motors[m].targetAv)
          {
             // we are done
             motors[m].stopFlag = 1;
             Serial.println("X1 " + String(motors[m].ticksLeft));
             Serial.println("Y " + String(motors[m].currentAv) + ":" + String( motors[m].targetAv));
             continue;
          }
        } 
        else
        {
          if (motors[m].currentAv < motors[m].targetAv)
          {
             // we are done
             motors[m].stopFlag = 1;
             Serial.println("X2 " + String(motors[m].ticksLeft));
             Serial.println("Y " + String(motors[m].currentAv) + ":" + String( motors[m].targetAv));
             continue;
          }
        }       
      ...
      else
      {
        ...
        if (m == 2)
        {
          if (motors[m].currentAv < motors[m].targetAv) 
          {
             // we are done
             motors[m].stopFlag = 1;
             Serial.println("X3 " + String(motors[m].ticksLeft));
             Serial.println("Y " + String(motors[m].currentAv) + ":" + String( motors[m].targetAv));
             continue;
          }
        }
        else
        {
          if (motors[m].currentAv >= motors[m].targetAv) 
          {
             // we are done
             motors[m].stopFlag = 1;
             Serial.println("X4 " + String(motors[m].ticksLeft));
             Serial.println("Y " + String(motors[m].currentAv) + ":" + String( motors[m].targetAv));
             continue;
          }       
        } 
...
int moveToPosition(char motor,int position)
{
    int moveAv,aVadj,moveTicks;

       Serial.println(String(position) + " " + String(motors[motor].avPerDegree));

        if (position > motorLimits[motor].upperLimit )
        {
          Serial.println ("ERR - too far");
          return 0;
        }
        
        if (position < motorLimits[motor].lowerLimit )
        {
        ...
        aVadj = int((position-motors[motor].defaultDegrees) * motors[motor].avPerDegree);
        
        //motors 0 and 1 have high Av as low degrees while motor 2 has low Av at low degrees
        
        motors[motor].targetAv = motors[motor].defaultAv + (motor == 2 ? +aVadj +20: -aVadj -20);
        motors[motor].currentAv = analogRead(motors[motor].positionAnalogPin);

        moveAv = motors[motor].targetAv - motors[motor].currentAv;
        moveTicks = (position * motors[motor].ticksPerDegree) - motors[motor].position;

        Serial.println(String(motors[motor].currentAv) + " " + String(motors[motor].targetAv) + " " + String(moveAv));
        
        if (1) //motor == 2)
        {
//          startMotor(motor,(moveAv >= 0 ? ROT_CW : ROT_CCW),abs(moveAv) * motors[motor].ticksPerAv,0);
          startMotor(motor,(moveTicks >= 0 ? ROT_CW : ROT_CCW),moveTicks,0);
        }
        else
        {
//           startMotor(motor,(moveAv < 0 ? ROT_CW : ROT_CCW),abs(moveAv) * motors[motor].ticksPerAv,0);
...
void startMotor(char motor, char cmd,int ticks, int repeat)
{
  ...
  //ticks = 4000;  //Serial.println(ticks);
